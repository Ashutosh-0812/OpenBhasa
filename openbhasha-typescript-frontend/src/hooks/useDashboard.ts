...
export const useDashboard = () => {
  ...
  // Load data on mount and when user changes (disabled until backend is available)
  useEffect(() => {
    if (user && isAuthenticated) {
      console.log('📊 Dashboard ready for user:', user.name);
      
      // Provide immediate mock data instead of loading from backend
      const mockStats = {
        totalProjects: 3,
        totalTasks: 8, 
        completedTasks: 5,
      ...
      // TODO: Enable this when backend is running
